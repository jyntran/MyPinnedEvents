...
function storeJson(userid, htmlId) {
  ...
  var model = {
    ...
    storeData: function() {
      ...
      $.ajax({
        type: "POST",
        url: wsURL + "/storeUserData/j44trana4/"+userid+"?key="+key,
        datatype: "json",
      ...
      $.ajax({
        type: "POST",
        url: wsURL + "/storeAppData/j44trana4?key="+key,
        datatype: "json",
        contentType: "application/json; charset=utf-8",
    ...
    getData: function() {
      var that = this;
      $.getJSON(wsURL + "/getUserData/j44trana4/"+userid+"?key="+key,
        function (d) {
      ...
      $.getJSON(wsURL + "/getAppData/j44trana4?key="+key,
        function (d) {
          if (d.meta.status === "200 OK") {
